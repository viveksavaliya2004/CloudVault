...
const processFileJob = async (job) => {
  ...
  if (file.mimeType.startsWith('image/')) {
    file.thumbnailPath = `/uploads/thumbnails/thumb_${file.fileName}`;
  } else if (file.mimeType.includes('pdf')) {
    file.thumbnailPath = `/uploads/thumbnails/pdf_preview_${file._id}.png`;
  } else {
    file.thumbnailPath = `/uploads/thumbnails/doc_icon.png`;
  }
  await file.save({ validateBeforeSave: false });
