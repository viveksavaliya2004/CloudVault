...
import {
  ...
  Activity, Clock, UserCheck, UserX, Calendar, ChevronLeft, ChevronRight
} from 'lucide-react';
...
// Custom Polished Modern Calendar Picker Dropdown
const CustomCalendarPicker = ({ value, onChange, label }) => {
  ...
  return (
    <div className="relative inline-block text-left select-none">
      <button
        ...
        className="flex items-center gap-2.5 px-3.5 py-2 text-xs font-semibold rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-900 transition-all cursor-pointer shadow-2xs focus:ring-2 focus:ring-indigo-500/30"
      >
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="absolute left-0 mt-2 w-64 p-3.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xl z-50 transition-colors duration-300"
          >
            {/* Calendar Header */}
            ...
            <div className="grid grid-cols-7 gap-1 text-center">
              {days.map((d, idx) => {
                if (d === null) return <div key={idx} className="h-7" />;
                  ...
                  <button
                    ...
                    className={`h-7 w-7 mx-auto rounded-lg text-xs font-semibold flex items-center justify-center transition-all cursor-pointer ${
                      isSelected
                        ...
                        : 'text-slate-700 dark:text-slate-300 hover:bg-indigo-50 dark:hover:bg-indigo-950/50 hover:text-indigo-600 dark:hover:text-indigo-400'
                    }`}
                  >
                    {d}
                  </button>
  ...
  return (
    <div className="space-y-6 sm:space-y-8 text-left select-none">
          ...
          <div className="flex-1 w-full space-y-6">
            ...
            {subTab === 'usage' && (
              <div className="space-y-6 w-full">
                {/* Modern Date Range Picker Bar */}
                <div className="flex items-center gap-4 flex-wrap bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 shadow-sm transition-colors duration-300">
                  <div className="flex items-center gap-3 flex-wrap">
                    ...
                    <CustomCalendarPicker
                      ...
                      onChange={(newStart) => {
                        ...
                        updateAnalytics(newStart, endDate);
                      }}
                      label="Start Date"
                    />
                    ...
                    <CustomCalendarPicker
                      ...
                      onChange={(newEnd) => {
                        setEndDate(newEnd);
                        updateAnalytics(startDate, newEnd);
                      }}
                      label="End Date"
                    />

                    {/* Quick Range Presets */}
                    <div className="flex items-center bg-slate-100 dark:bg-slate-800 p-0.5 rounded-xl text-[11px] font-semibold ml-2">
                      <button
                        type="button"
                        onClick={() => {
                          const start7 = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
                          const end7 = new Date().toISOString().split('T')[0];
                          setStartDate(start7);
                          setEndDate(end7);
                          updateAnalytics(start7, end7);
                        }}
                        className="px-2.5 py-1 rounded-lg text-slate-600 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-white transition-colors cursor-pointer"
                      >
                        7 Days
                      </button>
                      <button
                        ...
                        onClick={() => {
                          ...
                          setStartDate(start30);
                          setEndDate(end30);
                          updateAnalytics(start30, end30);
                        }}
                        className="px-2.5 py-1 rounded-lg text-slate-600 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-white transition-colors cursor-pointer"
                      >
                        30 Days
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center bg-slate-100 dark:bg-slate-800 p-0.5 rounded-xl text-[11px] font-semibold">
                        <button
                          ...
                          className={`px-3 py-1 rounded-lg transition-all duration-200 cursor-pointer ${sortBy === 'requests'
                        ...
                        <button
                          ...
                          className={`px-3 py-1 rounded-lg transition-all duration-200 cursor-pointer ${sortBy === 'bandwidth'
                            ? 'bg-indigo-600 text-white shadow-xs'
                            : 'text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200'
                            }`}
                        >
                ...
                >
                  <div className="space-y-4">
                    ...
                    <div className="space-y-3 pt-3">
                      <div className="flex justify-between text-xs font-medium">
                        ...
                        <span className="text-slate-800 dark:text-white font-bold">
                          {stats?.verifiedUsersCount || 0} users
                        </span>
                      </div>
                      <div className="w-full h-2 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
                        <motion.div
                          initial={{ width: 0 }}
                          animate={{ width: `${stats?.usersCount > 0 ? ((stats.verifiedUsersCount / stats.usersCount) * 100) : 0}%` }}
                          transition={{ duration: 0.8 }}
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="w-full h-2 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
                        <motion.div
                          initial={{ width: 0 }}
                          animate={{ width: `${stats?.usersCount > 0 ? ((stats.unverifiedUsersCount / stats.usersCount) * 100) : 0}%` }}
                          transition={{ duration: 0.8 }}
                  ...
                  <div className="p-3.5 rounded-xl border border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/20 text-xs text-slate-555 dark:text-slate-400 mt-4 leading-relaxed">
                    💡 Platform health: <span className="font-semibold text-slate-700 dark:text-slate-300">
                      {stats?.usersCount > 0 ? Math.round(((stats.verifiedUsersCount / stats.usersCount) * 100)) : 0}%
                    </span> of your userbase has verified their login identity.
                  </div>
                </motion.div>
              </div>
            )}
      ...
      {activeTab === 'logs' && (() => {
        ...
        const startLogIndex = (logPage - 1) * LOGS_PER_PAGE;
        const paginatedLogs = filteredLogs.slice(startLogIndex, startLogIndex + LOGS_PER_PAGE);

        return (
          <motion.div
            initial={{ opacity: 0, y: 12 }}
            animate={{ opacity: 1, y: 0 }}
            className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 transition-colors duration-300 space-y-5"
          >
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 dark:border-slate-800 pb-4">
              <div>
                ...
                <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">Real-time audit log history of platform actions</p>
              </div>
              ...
              <div className="flex items-center gap-3 flex-wrap">
                ...
                <input
                  ...
                  className="text-xs border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-200 rounded-xl px-3 py-1.5 w-full sm:w-48 focus:outline-none focus:ring-2 focus:ring-indigo-500/40"
                />
                ...
                <div className="flex items-center bg-slate-100 dark:bg-slate-800 p-0.5 rounded-xl text-[11px] font-semibold">
                  {[
                    { key: 'all', label: 'All' },
                    { key: 'file_upload', label: 'Uploads' },
                    { key: 'user_register', label: 'Registrations' },
                    { key: 'user_block', label: 'Blocks' }
                  ].map(f => (
                    <button
                      key={f.key}
                      onClick={() => {
                      ...
                      className={`px-2.5 py-1 rounded-lg transition-all duration-200 cursor-pointer ${logFilter === f.key
                        ? 'bg-indigo-600 text-white shadow-xs'
                        : 'text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200'
                        }`}
                    >
            ...
            {paginatedLogs.length === 0 ? (
              <div className="text-center py-10">
                ...
                <p className="text-sm text-slate-400 dark:text-slate-500">No activity logs matching your filter criteria</p>
              </div>
            ) : (
              <div className="space-y-2.5">
                {paginatedLogs.map(log => (
                  <div key={log.id} className="flex items-start justify-between gap-4 p-3.5 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 hover:bg-slate-100/60 dark:hover:bg-slate-800/80 transition-colors">
                    ...
                    <span className="text-xs font-semibold text-slate-400 dark:text-slate-500 flex-shrink-0 whitespace-nowrap">
            ...
            {totalLogPages > 1 && (
              <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
                <div>
                  Showing <span className="font-semibold text-slate-700 dark:text-slate-200">{startLogIndex + 1}</span> to <span className="font-semibold text-slate-700 dark:text-slate-200">{Math.min(startLogIndex + LOGS_PER_PAGE, filteredLogs.length)}</span> of <span className="font-semibold text-slate-700 dark:text-slate-200">{filteredLogs.length}</span> logs
                </div>
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <div className="flex items-center gap-1 px-2 font-medium">
                    <span>Page</span>
                    <span className="font-bold text-slate-800 dark:text-white">{logPage}</span>
                    <span>of</span>
                    <span className="font-bold text-slate-800 dark:text-white">{totalLogPages}</span>
                  </div>
